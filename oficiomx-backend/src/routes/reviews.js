...
function adminOnly(req, res, next) {
  ...
  // Ayuda al diagnóstico en desarrollo
  if (!expected) {
    console.warn('[Admin] ADMIN_TOKEN no está definido en .env')
    return res.status(500).json({ ok: false, error: 'ADMIN_TOKEN no configurado en el servidor' })
  }
...
router.get('/pending', adminOnly, (req, res) => {
  // trades es un JSON array — lo devolvemos parseado
  const rows = db.prepare(`
    SELECT r.id, r.contractor_id, r.author_name, r.author_email,
           r.stars, r.body, r.status, r.created_at,
           c.name AS contractor_name, c.trades
    FROM reviews r
  ...
  const reviews = rows.map(r => ({
    ...r,
    trades: (() => { try { return JSON.parse(r.trades) } catch { return [r.trades] } })(),
    // trade legible para mostrar en el panel
    trade: (() => { try { return JSON.parse(r.trades).join(', ') } catch { return r.trades } })(),
  }))
...
router.patch('/:id/approve', adminOnly, (req, res) => {
  // sql.js no expone changes directamente — verificamos con SELECT
  ...
  if (review.status !== 'pending')
    return res.status(409).json({ ok: false, error: 'La reseña ya fue procesada' })

  db.prepare(`
    UPDATE reviews SET status = 'approved', approved_at = datetime('now')
    WHERE id = ?
  `).run(req.params.id)
