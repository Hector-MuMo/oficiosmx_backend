...
function parse(c) {
  ...
  try {
    c.coverage_zones = JSON.parse(c.coverage_zones);
  } catch {
    c.coverage_zones = [];
  }
  return c;
...
router.get("/", (req, res) => {
  ...
  if (zone) {
    query +=
      " AND (LOWER(c.zone) LIKE LOWER(?) OR LOWER(c.coverage_zones) LIKE LOWER(?))";
  ...
  // Solo agregar ORDER BY si se proporciona sort
...
router.get("/:id", (req, res) => {
  ...
  if (!c)
    return res
      .status(404)
      .json({ ok: false, error: "Contratista no encontrado" });
  c.reviews = db
    .prepare(
      `
...
router.post("/", (req, res) => {
  ...
  if (!Array.isArray(trades) || trades.length === 0 || trades.length > 4)
    return res
      .status(400)
      .json({ ok: false, error: "Selecciona entre 1 y 4 oficios" });

  const wa = String(whatsapp).replace(/\D/g, "");
  if (wa.length < 10)
    return res
      .status(400)
      .json({ ok: false, error: "Número de WhatsApp inválido" });
